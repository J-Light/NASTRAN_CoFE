...
% find property
pidH = find([FEM.PBAR.PID]==obj.PID);
if size(pidH,2) > 1; error(['There should be one and only one PBAR or PBARL with ID# ',num2str(obj.PID),'']); end
if size(pidH,2) == 1
    obj_pbeam = FEM.PBAR(pidH);
% else
%     pidH = find([FEM.PBARL.PID]==obj.PID);
...
if isnan(obj.G0)
    ...
    g0 = find(FEM.gnum==obj.G0);
    if size(g0,2)~=1; error(['There should be one and only one GRID with ID# ',num2str(obj.G0),'']); end
    nuVec = [FEM.GRID(g0).X1 - obj.x1(1) ; FEM.GRID(g0).X2 - obj.x1(2) ; FEM.GRID(g0).X3 - obj.x1(3)];    
end
